...
void slcan_parse_str_status(uint8_t *buf, uint8_t len)
{
    ...
    if (can_get_bus_state() == ON_BUS)
    {
        ...
        else if (buf[0] == 'f')
        {
            ...
            snprintf(dbgstr, 128, "f: node_sts=%s, error_code=%s, error_cnt_tx_rx=[0x%02X, 0x%02X], est_bus_load_percent=%02d\r",
                                        ...
                                        (sts.LastErrorCode == FDCAN_PROTOCOL_ERROR_ACK ? "_ACK" : 
                                        (sts.LastErrorCode == FDCAN_PROTOCOL_ERROR_BIT1 ? "BIT1" : 
                                        (sts.LastErrorCode == FDCAN_PROTOCOL_ERROR_BIT0 ? "BIT0" : 
                                        (sts.LastErrorCode == FDCAN_PROTOCOL_ERROR_CRC ? "_CRC" : "SAME"))))))),
